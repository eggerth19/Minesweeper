...
function generateGame() {
    ...
    for (let ii = 0; ii < board.rows; ii++) {
        ...
        for (let i = 0; i < board.cols; i++) {
            ...
            cells[cellnr] = {id:cellnr, x:ii, y:i, isNotClicked:true, isNotFlaged:true, itIsBomb:isBomb(ii,i)}
            if(cells[cellnr].itIsBomb){
                bombs.push(cellnr)
            }
            minezone.innerHTML += '<div class="cell" onclick="doStuff('+cellnr+')"  oncontextmenu="setFlag('+cellnr+')" id="cell_'+cellnr+'"></div>'
            cellnr++   
        };
...
function isNextToBomb(nbr,cnt=0){
    ...
    let neighb = [nbr-1, nbr-rows-1, nbr-rows, nbr-rows+1, nbr+1, (nbr+1)+rows, nbr+rows, nbr+(rows-1)]
    neighb.forEach(neighb_cell => {
        //so we don't count cells that are not within the board, f.x. negative number cells
        if (cells[neighb_cell] && cells[neighb_cell].y>=0 && cells[neighb_cell].y+1<=board.cols && cells[neighb_cell].itIsBomb){ 
            //so we don't count cells on the far left side as neighbours of cells on the far right side
            if(cells[neighb_cell].y - cells[nbr].y < 2 && cells[neighb_cell].y - cells[nbr].y > -2){
...
function doStuff(nbr){
    if(gameON && cells[nbr].isNotFlaged){
        ...
        //if we click a cell with a neighbouring mine
            ...
            if(near_bombs >= 3)
                {currentid.innerHTML='<DIV class="cellfix hint hint3">'+near_bombs+'</DIV>'
            }
            else if (near_bombs == 2)
                {currentid.innerHTML='<DIV class="cellfix hint hint2">'+near_bombs+'</DIV>'
            }
            else
                {currentid.innerHTML='<DIV class="cellfix hint">'+near_bombs+'</DIV>'
            }
        }
        //for all other cells we recursively open them
                ...
                neighb.forEach(neighb_cell => {
                    if(cells[neighb_cell] && cells[neighb_cell].isNotClicked && cells[neighb_cell].itIsBomb == false){
...
function boom(){
    ...
    for (let i = 0; i < bombs.length; i++){
        ...
        bombid.innerHTML='<DIV class="cellfix"><img src="bomb.png" alt="B" class="img_bomb"></DIV>'
    }
...
function setFlag(nbr){
    ...
    if(cells[nbr].isNotClicked && gameON == true){
        ...
        flagid.innerHTML='<DIV class="cellfix"><img src="flag.png" alt="F" class="img_flag"></DIV>'
...
function doAjax() {
    ...
    axios.post(url, { rows: r, cols: c, mines: m })
        .then(function (response) {
            ...
            console.log(board)
            //generateGame()
            genbutton.style.backgroundColor = "LightGrey"
            genbutton.innerHTML = "Generate new Game"
        })
        .catch(function (error) {
            //if the connection to the url fails, we generate an 10x10 board with mines in fixed positions
            board = {rows:10, cols:10, mines:10, minePositions:[[1,3],[3,0],[4,2],[4,5],[4,7],[6,9],[7,7],[8,9],[9,3],[9,9]]}
            //generateGame()
            genbutton.style.backgroundColor = "LightCoral"
            genbutton.innerHTML = "The Game dit not generate correctly - Default board generated"
        })
        .then(function () {
            generateGame()
        });
